...
- 首页条件筛选：连载中 / 有亲吻 / 有露点，可多选叠加
- 章节记录支持折叠模式（默认，只显示话数色块）与详细模式（完整文字记录）
- 支持用户提交数据：分「新增漫画」和「修改已有漫画」两类；新增漫画按流程填写标题/连载状况/作者/几章并编辑章节色块；修改已有漫画可逐章修改色块并注明原因，从无/未知改为有时必须附带佐证图片；每张佐证图片必须备注对应章节
